...
void PlinkMatrix::ReadCompact(int* variant_subset,
    ...
    if (plink2::cachealigned_malloc(byte_ct * vsubset_size, &compactM)) {
        stop("Out of memory");
    }
...
double PlinkMatrix::dot_product(int j, const double* v) {
    ...
    const uint32_t word_ct =
        plink2::DivUp(_subset_size, plink2::kNypsPerCacheline) *
        plink2::kWordsPerCacheline;
...
double PlinkMatrix::vx2(int j, const double* v) {
    ...
    const uint32_t word_ct =
        plink2::DivUp(_subset_size, plink2::kNypsPerCacheline) *
        plink2::kWordsPerCacheline;
    double result = plink2::LinearCombinationSquare(v, &(compactM[j * word_ct]),
                                                    _subset_size, xm[j]);
    result /= (xs[j] * xs[j]);
    return result;
}
...
void PlinkMatrix::update_res(int j, double d, const double* v, double* r) {
    const uint32_t word_ct =
        plink2::DivUp(_subset_size, plink2::kNypsPerCacheline) *
        plink2::kWordsPerCacheline;
    plink2::update_res_raw(&(compactM[j * word_ct]), d, v, r, _subset_size,
                           xm[j], xs[j]);
}
...
void PlinkMatrix::multiply_vector(const double* v, double* r) {
    ...
    for (int j = 0; j < _vsubset_size; ++j) {
        plink2::update_res2_raw(&(compactM[j * word_ct]), v[j], r, _subset_size,
                                xm[j], xs[j]);
    }
    return;
}
