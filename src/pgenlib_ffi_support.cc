...
double genoarrproduct(const uintptr_t* genoarrx, const uintptr_t* genoarry, uint32_t sample_ct)
{
  const uint32_t word_ct = DivUp(sample_ct, kBitsPerWordD2);
  double result = 0.0;
  for (uint32_t widx = 0; widx != word_ct; ++widx)
  {
    const uintptr_t genox_word = genoarrx[widx];
    const uintptr_t genoy_word = genoarry[widx];
    uintptr_t genox_word1 = genox_word & kMask5555;
    uintptr_t genox_word2 = (genox_word >> 1) & kMask5555;
    uintptr_t genox_missing_word = genox_word1 & genox_word2;
    genox_word1 ^= genox_missing_word;
    genox_word2 ^= genox_missing_word;

    uintptr_t genoy_word1 = genoy_word & kMask5555;
    uintptr_t genoy_word2 = (genoy_word >> 1) & kMask5555;
    uintptr_t genoy_missing_word = genoy_word1 & genoy_word2;
    genoy_word1 ^= genoy_missing_word;
    genoy_word2 ^= genoy_missing_word;

    uintptr_t both_1 = genox_word1 & genoy_word1;
    uintptr_t both_2 = genox_word2 & genoy_word2;
    uintptr_t one_and_two = (genox_word1 & genoy_word2) | (genoy_word1 & genox_word2);
    uint32_t xmissing_weight = plink2::PopcountWord(genox_missing_word & genoy_word1) + 2*plink2::PopcountWord(genox_missing_word & genoy_word2);
    uint32_t ymissing_weight = plink2::PopcountWord(genoy_missing_word & genox_word1) + 2*plink2::PopcountWord(genoy_missing_word & genox_word2);
    uint32_t both_missing_count = plink2::PopcountWord(genox_missing_word & genoy_missing_word);

    result += (double)(plink2::PopcountWord(both_1) + 2*plink2::PopcountWord(one_and_two) + 4*plink2::PopcountWord(both_2));
    double xmean = 0.0;
    double ymean = 0.0;

    if((both_missing_count > 0) || (xmissing_weight > 0) )
    {
      STD_ARRAY_DECL(uint32_t, 4, genocountsx);
      GenoarrCountFreqsUnsafe(genoarrx, sample_ct, genocountsx);
      const double numer = u63tod(genocountsx[1] + 2 * genocountsx[2]);
      const double denom = u31tod(sample_ct - genocountsx[3]);
      xmean = numer/denom;
    ...
    if((both_missing_count > 0) || (xmissing_weight > 0))
    {
      ...
      const double numer = u63tod(genocountsy[1] + 2 * genocountsy[2]);
      const double denom = u31tod(sample_ct - genocountsy[3]);
      ymean = numer/denom;
      result += ((double)ymissing_weight) * ymean;
    }

    result += both_missing_count * xmean * ymean;
  }
  return result;
}
